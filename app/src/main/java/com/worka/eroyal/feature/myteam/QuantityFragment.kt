...
import com.worka.eroyal.databinding.FragmentQuantityBinding
 ...
 * on 11/06/20.
 */
class QuantityFragment: BaseFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        with(viewModel) {
            salesQtyItemViewModel.observe(viewLifecycleOwner, Observer {
                adapter?.list = it
    ...
    private fun setupBrandFilterDialog() {
        context?.let { ctx ->
            brandFilterDialog = CheckBoxFilterDialog(
                ...
                viewModel.quantityBrandFilterList
            ) {
                ...
                with(viewModel) {
                    quantityBrandFilterList = list
                    setQuantityBrandFilterTitle()
                }
                getData()
            }
        }
